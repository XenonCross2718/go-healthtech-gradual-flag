...
Infrai keeps this example to one small HTTP client and a single `INFRAI_API_KEY` covering the flag calls. The request code is ordinary Go, so the same shape is easy to place beside an existing service.
The endpoint is plain REST from any language; this repository shows the Go form.
